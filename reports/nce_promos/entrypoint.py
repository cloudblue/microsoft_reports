...
def generate(
    ...
    for request in requests:
        ...
        nce_promo_final = parameter_value(
            'nce_promo_final',
...
def _get_tier1_mpn(client, product_id, account_id):
    ...
    if account_id not in TIER_CONFIGS[product_id]:
        ...
        config = client.ns('tier').collection('configs').filter(query).select(
            '-connection',
            '-configuration',
            '-contract',
            '-marketplace',
...
def _get_promo_applied(item, promo_final):
    ...
    if promo_final and promo_final != "-" and promo_final['promotions'][0]['mpn'] == item_mpn:
...
def _process_line(client, request, item, promotion):
    ...
    return (
        ...
        parameter_value('ms_customer_id', get_value(request, ['asset', 'params'], [])),
        parameter_value('microsoft_domain', get_value(request, ['asset', 'params'], [])),
        parameter_value('subscription_id', get_value(request, ['asset', 'params'], [])),
        parameter_value('csp_order_id', get_value(request, ['asset', 'params'], [])),
        get_value(item, ['mpn']),
